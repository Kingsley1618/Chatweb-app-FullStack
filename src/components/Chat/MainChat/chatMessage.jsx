...
const {lastDiv} = useScrolltoView()
  ...
  return (
    <div className='flex-1 flex flex-col gap-y-2 px-3' style={{overflowY:"auto"}}>
     ...
     <div className='flex gap-x-2 items-center'>  <Image src={message?.data()?.photoUrl} alt="profile" className="rounded-[50%] w-[30px] h-[30px]" height={50} width={50}/>
         
        
   <h1 className='font-black text-[black] text-[1rem]'>{message?.data()?.firstName} {message?.data()?.lastName}</h1> 
         
{userId === message?.data()?.id ? <AiOutlineClose className="ms-auto cursor-pointer text-[.6rem] text-black font-bold" onClick = {
